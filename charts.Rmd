...
packages = c('tidyverse', 'tidytext', 'dplyr', 'ggplot2','plyr','gridExtra','ggpubr','reshape2')
...
p1_u <- ggplot(weighted_un, mapping=aes(x=Var2, 
  ...
  geom_bar(stat="identity",fill="brown") +
           scale_y_continuous(labels = scales::percent, limits = c(-.40,.20)) +
  labs(
...
p3_u <- ggplot(df_long, aes(fill=university, y=perchang, x=Var2)) + 
  ...
  scale_fill_manual(labels = c("Public \nUniversity", "Private &\nUniversity"), values = c("per_change_pu" = "#66CC99", "per_change_pr" = "purple")) +
           scale_y_continuous(labels = scales::percent, limits = c(-.40,.20)) +
  labs(
...
p4_c <- ggplot(df_long, aes(fill=org, y=perchang, x=Var2)) + 
    geom_bar(position="stack", stat="identity") + 
  scale_fill_manual(labels = c("Civil \nSociety", "Research &\nBusiness"), values = c("per_change_ci" = "orange", "per_change_re" = "blue")) +
           scale_y_continuous(labels = scales::percent, limits = c(-.40,.20))+
  labs(
...
ggarrange(p3_u, p4_c, ncol=2, nrow=1, common.legend = FALSE, legend="bottom") %>%
  ggexport(filename = "diagrams/figure7B.png")
```
```{r}
#final plot export for Figure 7. B
...
ggbarstats(
  ...
  labs(
    title = "Author Stakeholder Groups of SDG Trainings",
    subtitle = "per Sustainable Governance Capacity",
    x = "Sustainable Governance Capacities",
    y = "Contributing Author Institions per Group",
    fill = NULL
  ) + theme(legend.position = "none") + theme_minimal()
