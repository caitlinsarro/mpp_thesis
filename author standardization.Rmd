...
urls_with_cate <-merge(results_urls_t, au_category_clean, by.x = 1, by.y = "id", all.x = TRUE)
...
un_programs <- 'UN-Habitat|UNDP|UNEP|UN Global Compact'

df <- data.frame(authors_list_A$authors, un_programs = 0)
...
df_agent[grep(un_specialized_agencies, df_agent$authors, value = F), "un_specialized_agencies"] <- 1
...
ngo <-c('(c)(3)|501|nonprofit|non-profit|non-governmental|nongovernmental|Non-governmental|NGO|ngo')
df_ngo <- data.frame(authors_list_A$authors, ngo = 0)
df_ngo[grep(ngo, df_ngo$authors, value = F), "ngo"] <- 1
...
authors_list_A$un_programs <- merge(authors_list_A, df, by.x = "authors_list_A$authors", by.y = "df$authors_list_A.authors", all.x = TRUE)

authors_list_A_comb$un_specialized_agencies <- merge(authors_list_A_comb, df_agent, by = "authors_list_A.authors", all = TRUE)

authors_list_A_comb$ngo <- merge(authors_list_A_comb, df_ngo, by.x = "authors_list_A.authors", all = TRUE)

authors_list_A_comb$un <- merge(authors_list_A_comb, df_un, by.x = "authors_list_A.authors", all = TRUE)
...
authors_list_A_comb$category[authors_list_A_comb$authors=="Alliance for Global Water Adaptation"] <- "ingo"
...
authors_list_A_comb$category[authors_list_A_comb$category=="philanthropic initiative"] <- "ngo"
...
authors_list_A_comb$category[authors_list_A_comb$category=="Foundation"] <- "ngo"
