...
indicators <- c("$\\bullet$  stakeholder engagement\n  $\\bullet$ Indigenous\n $\\bullet$ facilitation methods", "* integrated planning 1\n * horizontal alignment \n * policy integration", "* direct democracy 1\n * governmental oversight 1\n * diplomatic skills", "* strategic planning \n * evidence based policy 1\n * ig data")

df <- data.frame(themes, examples, subthemes, indicators)


df %>%   
  kbl(caption = "Figure 3. Something Something", col.names=c("Themes","Examples","Sub themes","Example indicators")) %>%
...
```{r}
df_2 %>%
  kbl(col.names=linebreak(c('Criteria','Examples','Author Score\n for Civil Society'))) %>%
  kable_styling()
```
```{r}
df_2 %>%
  kbl(caption = "Figure 3. Something Something", col.names=c("Criteria","Examples","Author Score\n for Civil Society")) %>%
  kable_classic(full_width = F, html_font = "Open Sans") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"))
