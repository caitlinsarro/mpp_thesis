...
```{r}
#fixing nas
new_DF <- au_diff_coding[rowSums(is.na(au_diff_coding)) > 0,]
write.csv(new_DF,"C:/Users/caitl/Documents/GitHub/mpp_thesis/exports/deletelater.csv", row.names = TRUE)
...
#simply categories
au_diff_main_long$au_category[au_diff_main_long$au_category == "cultural institution"] <- "civil society"
au_diff_main_long$au_category[au_diff_main_long$au_category == "ngo"] <- "civil society"
au_diff_main_long$au_category[au_diff_main_long$au_category == "business"] <- "research  business"
au_diff_main_long$au_category[au_diff_main_long$au_category == "finance"] <- "research  business"
au_diff_main_long$au_category[au_diff_main_long$au_category == "research policy institutes"] <- "research  business"
au_diff_main_long$au_category[au_diff_main_long$au_category == "un ngo"] <- "civil society"
...
table(au_diff_allcourses$au_category, au_diff_allcourses$max.col)


chisq.test(au_diff_allcourses$au_category, au_diff_allcourses$max.col, correct=FALSE)

#X-squared = 41.712, df = 15, p-value = 0.0002489
authors <- au_diff_allcourses$au_category
govern <- au_diff_allcourses$max.col

#freq_table <- au_diff_allcourses %>% select(id.x, au_category, max.col)

#freq_table <- na.omit(freq_table)
freq_table <- table(au_diff_allcourses$au_category, au_diff_allcourses$max.col)
```

```{r}
#mosaic plot
au_diff_allcourses
mosaic(~ max.col + au_category, data = au_diff_allcourses,
...
au_diff_allcourses_simple <- au_diff_allcourses %>% select(id.x, au_category, max.col)
...
ggbarstats(
  au_diff_allcourses_simple,au_category,max.col,
  results.subtitle = FALSE,
  subtitle = paste0(
    "Fisher's exact test", ", p-value = ",
    ifelse(fishtest$p.value < 0.001, "< 0.001", round(fish$p.value, 3))
  )
)
